...
# %%
covid_path = '/home/scientist617/Grad_Proj/Data/new_sjogren_file/'

path = '/home/Backup3/DLproject/namphil_codes/all_vaccination'
files = ['pre-35.tsv', '1st-35.tsv', '2nd-35.tsv', '3rd-35_merged.tsv', '4th-35.tsv', '5th-35.tsv']
...
motifs = [
    ...
    Seq('ARDLQEAGGMDV'),
    Seq('ARDLLEAGGMDV'),
    Seq('ARVRGYSGYGANDAFDI'),
    Seq('ARVHGYSGYGANDAFDL'),
    Seq('ARVRGYSDYGANDAFDI')
]

motif_types = ['CDR3', 'CDR3', 'CDR3', 'CDR3', 'CDR3', 'CDR3', 'CDR3']
    ...
    for file in files:
        ...
        def update_score(df):
            ...
            for i, row in df.iterrows():
                ...
                if type == 'CDR1':
                    if q == 1:
                        similarity_scoreq = similarity_score_q * pow(sequence_similarity(
...
def run_script(q):
    ...
    pd.DataFrame([get_motif_score(path, motif, motif_types[i], q) for i, motif in enumerate(motifs)]).to_csv(
        f'/home/scientist617/Grad_Proj/Data/CoVID_Crossval/covid_score_q_{q}.csv', index=False)
